...
public class QuestionResource {
    ...
    public ResponseEntity<List<Question>> getAllQuestions(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        ...
        if(queryParams.size()>0){
            String userID = queryParams.toSingleValueMap().get("queryParams");

            Map<String, List<Answer>> answerList = answerRepository.findAllByUserId(userID).stream()
                .collect(groupingBy(Answer::getAnswerChoiseId));

            page.getContent().forEach( question -> {
                question.getAnswerChoises().forEach( answerChoice -> {
                    if(answerList.containsKey(answerChoice.getId())) {
                       Answer selectedAnswer =  answerList.get(answerChoice.getId()).get(0);
