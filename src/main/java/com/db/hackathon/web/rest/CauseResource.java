...
public class CauseResource {
    ...
    public ResponseEntity<List<Cause>> getAllCauses(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.info("REST request to get a page of Causes");
        Page<Cause> page = causeService.findAll(pageable);
        List<String> ids = page.get().map(cause -> cause.getId()).collect(Collectors.toList());
        List<Transaction> transactions = transactionService.findAllByCauseIds(ids);

        Map<String, Map<String, Long>> causeToTransactionsNoMap = transactions.stream()
            .collect(Collectors.groupingBy(Transaction::getCauseId, Collectors.groupingBy(Transaction::getUserId, Collectors.counting())));

        page.get().forEach(cause ->  {
            causeToTransactionsNoMap.putIfAbsent(cause.getId(), Collections.emptyMap());
            cause.setTotalUsers(Long.valueOf(causeToTransactionsNoMap.get(cause.getId()).size()));
            log.info("Cause {} - {} users", cause.getTitle(), cause.getTotalUsers());
        });
