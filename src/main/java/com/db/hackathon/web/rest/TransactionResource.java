...
/**
 * REST controller for managing {@link com.db.hackathon.domain.Transaction}.
 */
@RestController
@RequestMapping("/api")
public class TransactionResource {
    ...
    private static final String ENTITY_NAME = "helloworldTransaction";
    ...
    public ResponseEntity<Transaction> createTransaction(@RequestBody Transaction transaction) throws URISyntaxException {
        ...
        if(transaction.getCauseId() == null)
        {
            throw new BadRequestAlertException("A new transaction cannot have a null causeId", ENTITY_NAME, "causeidisnull");
        }

        Cause cause = causeService.findOne(transaction.getCauseId()).orElse(null);
        if(cause == null) {
            throw new BadRequestAlertException(String.format("Cause not found for causeId %s. Transaction cannot be created", transaction.getCauseId()), ENTITY_NAME, "causedoesntexist");
        }

        if(transaction.getUserId() == null)
        {
            throw new BadRequestAlertException("A new transaction cannot have a null userId", ENTITY_NAME, "useridisnull");
        }
        Status status = statusService.findByUserId(transaction.getUserId()).orElse(null);

        if(status == null) {
            throw new BadRequestAlertException(String.format("Status not found for userId %s. Transaction cannot be created", transaction.getUserId()), ENTITY_NAME, "statusdoesntexist");
        }

        Transaction result = transactionService.save(transaction);
        //add ammount to cause;
        cause.setCurrencyAmount(cause.getCurrencyAmount() + transaction.getTransactionAmount());
        causeService.save(cause);

        //take ammount from status;
        status.setCurrencyAmount(status.getCurrencyAmount() - transaction.getTransactionAmount());
